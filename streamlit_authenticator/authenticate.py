...
class Authenticate:
    """
    这个类将创建登录、注销、注册用户、重置密码、忘记密码，
    忘记用户名，并修改用户详细信息部件。
    """
    def __init__(self, credentials: dict, cookie_name: str, key: str, cookie_expiry_days: int=30, 
        preauthorized: list=None):
        """
        创建“Authenticate”的新实例。

        因素
        ----------
        credentials: dict全权证书:字典
            用户名、姓名、密码和电子邮件字典。
        cookie_name: str
            存储在客户端浏览器上用于无密码重新身份验证的JWT cookie的名称。
        key: str关键字:str
            用于对JWT cookie的签名进行哈希运算的密钥。
        cookie_expiry_days: int
            cookie在客户端浏览器上过期前的天数。
        preauthorized: list预授权:列表
            授权注册的未注册用户的电子邮件列表。    
        """
